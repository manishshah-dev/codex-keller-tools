        ...
        <div class="max-w-7xl mx-auto">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('ai-settings.prompts.store') }}" method="POST" class="space-y-6">
                        @csrf
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-input-label for="feature" :value="__('Feature')" />
                                <select id="feature" name="feature" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                    <option value="">Select Feature</option>
                                    @foreach($features as $key => $value)
                                        <option value="{{ $key }}" {{ old('feature') == $key ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <textarea id="prompt_template" name="prompt_template" rows="10" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>{{ old('prompt_template') }}</textarea>
                        ...
                        <!-- Hidden input for parameters -->
                        <input type="hidden" name="parameters" value="{{ json_encode([]) }}">
