...
class CandidateController extends Controller
{
    ...
    public function projectIndex(Project $project, ModelRegistryService $modelRegistryService, WorkableService $workableService): View // Inject service
    {
        ...
        // Fetch Workable jobs and candidates if settings available
        $workableCandidates = [];
        $workableJobs = [];
        $departments = [];
        $countries = [];
        ...
        if ($workableSetting) {
            try {
                $workableJobs = $workableService->listJobs($workableSetting);
                foreach ($workableJobs as $job) {
                    if (!empty($job['department'])) {
                        $departments[$job['department']] = $job['department'];
                    }
                    $country = $job['location']['country'] ?? ($job['location']['country_name'] ?? null);
                    if ($country) {
    ...
    public function importFromWorkable(Request $request, Project $project, WorkableService $workableService): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'workable_job' => 'nullable|string',
            'filter_email' => 'nullable|email',
